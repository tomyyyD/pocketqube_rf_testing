...
from lib.radio_utils import image_queue as iq
...
def request_image(task, time):
    if not iq.empty() and file_exists(iq.peek()):
        pass
    else:
        task.debug(f'no image queued at {time}')
        tq.push(Message(9, b'no image queued for', with_ack=True))
...
commands = {
    ...
    REQUEST_IMAGE: {"function": request_image, "name": "REQUEST_IMAGE", "will_respond":True, "has_args": True},
}
